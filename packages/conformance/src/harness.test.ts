...
import { runSuites, suite, titleError } from "./suite.js";
...
runSuites(target, [
  suite("harness", (test) => {
    test("harness: a fresh world registers the organiser and every holder, each self-signed", async (world) => {
    ...
    test("harness: the SDK runs on the backend's settable clock and seeded randomness", async (world) => {
