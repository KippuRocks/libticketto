...
//
// The port frames signed inputs with `@ticketto/profile-v0`, whose `scale-ts`
// constructs a `TextDecoder` when imported. Hermes ships `TextEncoder` but not
// `TextDecoder`, so an app embedding the binding installs one first, as
// `profile-v0`'s own Hermes harness and F-030 do. This is a minimal UTF-8
// decoder standing in for that polyfill. The C4 client itself needs none.
