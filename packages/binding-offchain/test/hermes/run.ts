// Runs the portable suites of binding-offchain under the Hermes VM, as
// profile-v0/test/hermes/run.ts does for the profile.
//
...
const VECTORS_MODULE = "ticketto-c4-vectors";
...
function writeVectorsModule(): string {
  ...
  const json = readFileSync(join(packageRoot, "test", "c4", "c4-v0.json"), "utf8");
  writeFileSync(path, `globalThis.__TICKETTO_C4_VECTORS__ = ${json.trim()};\n`);
  return path;
}
    ...
    mergeConfig(getDefaultConfig(hermesDir), {
      ...
      resolver: {
        ...
        resolveRequest: (context, moduleName, platform) => {
          // The vendored C4 vectors, as a module that publishes them on globalThis.
