// Byte helpers for the log's codecs.
//
// Identifiers cross the SDK surface as lower-case hex of their canonical bytes
// (features/002-sdk/plan.md §5.1). Parsing is strict, so one identifier has one
// string form.
...
/** The bytes of lower-case hex; a `TypeError` for anything else or, given `length`, another length. */
...
/** Whether `a` and `b` hold the same bytes. Not constant-time: public values only. */
